...
export const DisplayCars = ({userImage, data, displayMsg}) => {
  ...
  const colorList = ["white", "black", "blue", "red", "green", "yellow", "silver", "gold"];
const carTypes = ["sedan", "suv", "coupe", "convertible", "hatchback", "wagon", "van", "minivan", "pickup truck", "crossover", "luxury car", "luxury vehicle", "mid-size car"];
  ...
  <div className="flex flex-col md:flex-row items-center justify-center">
      ...
      {data && userImage ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-12">
          <CarCard userImage={userImage} userData={data} displayMsg={displayMsg} />
          {carDb
            ?.filter((entry) => entry.carType === inputCar.carType || entry.carColor === inputCar.carColor)
            .map((entry) => (
              <MatchCard
                key={entry.imgUrl}
                imgUrl={entry.imgUrl}
    ...
    <div className="bg-slate-200 rounded-xl text-slate-900 pt-2 overflow-hidden">
      {userImage && userData && (
        <section className="flex flex-col px-4">
            ...
            <img src={userImage} alt={userImage} className="w-full h-auto object-cover curser-pointer px-4"/>
...
export const MatchCard = ({imgUrl, carType, carColor, carBrand, inputCar}) => {
  ...
  return(
    ...
    {imgUrl &&
      <section className="flex flex-col px-4">
        ...
        <img src={imgUrl} alt={imgUrl} className="w-full h-auto object-cover curser-pointer px-4"/>
